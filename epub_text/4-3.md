...
[CHAPTER 4-2の「ロールの上書き」](4-2.xhtml#ロールの上書き)で説明したとおり、`role`属性を指定すると、要素のセマンティクスを上書きして変更できる場合があります。
...
ラベル付け可能要素の場合は、関連付けられた[`label`要素<!--(P212)-->](3-10.xhtml#label要素)の内容もアクセシブルな名前となります。また、以下の属性によっても名前を与えることができます。

- [`aria-labelledby`属性<!--(P302)-->](4-2.xhtml#aria-labelledby属性プロパティ)
- [`aria-label`属性<!--(P301)-->](4-2.xhtml#aria-label属性プロパティ)
- [`title`属性<!--(P278)-->](4-1.xhtml#title属性)
...
このテクニックは、先に紹介した[「操作可能な要素を隠さない」<!--(P311)-->](#操作可能な要素を隠さない)という考え方と競合することに注意してください。原則として、操作可能な要素に適用すべきではありません。\[20]は不適切な例で、[CHAPTER 3-6で紹介した「スキップリンク」](3-6.xhtml#スキップリンク)に`visually-hidden`を適用しています。
...
通常、フォーカス可能でない要素はキーボードフォーカスを受け取れませんが、[CHAPTER 4-1で紹介した`tabindex`属性](4-1.xhtml#tabindex属性)を利用すると、フォーカスを受け取れるようになります。
...
[^18]: ユーザー操作を実装する際は、キーボード操作にも対応できるように配慮する必要があります。[「マウスやタッチで操作可能なものは、キーボードでも操作可能にする」<!--(P310)-->](#マウスやタッチで操作可能なものはキーボードでも操作可能にする)も参照してください。
