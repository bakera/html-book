...
CHAPTER 2-1で触れたように、HTML文書はDOCTYPEで始まります。通常は、その直後に`<html>`タグが出現することになります。[01]のように、コメントを間に挟むこともできます。
...
`lang`属性は、要素内の言語(自然言語)を指定するグローバル属性です。詳細はCHAPTER 4-1を参照してください。
...
CHAPTER 1-3で触れたように、HTML文書はXML構文としても記述できます[^2]。この場合、[03]のように`xmlns`属性を指定します。これは名前空間宣言と呼ばれ、XMLの要素や属性がどのマークアップ言語由来なのかを区別するためのものです。

[^2]: CHAPTER 1-6で触れたように、MIMEタイプapplication/xhtml+xmlでHTML文書を配信するとXML構文であるとみなされます。MIMEタイプtext/htmlで配信される通常のHTML構文では、`xmlns`属性は不要です。指定自体は可能ですが、属性値として"http://www.w3.org/1999/xhtml"以外を指定した場合は構文エラーになります。
...
`title`要素はページのタイトルを表すものです。原則として、この要素は`head`要素内に必ず1つ存在しなければなりません[^5]。
...
<figure>
<figcaption>[04] `title`要素にタブを含めた記述例</figcaption>
```html
...
`base`要素を利用すると、基準URLや、リンク先を表示する*デフォルトのウィンドウターゲット*を指定できます。この要素はなくても構いませんが[^9]、存在する場合、1つしか存在してはなりません。
...
`target`属性は、リンク先をどのフレームやタブ、ウィンドウで開くかの デフォルト値を指定します。属性値の詳細は、`a`要素の`target`属性<!--(P165)-->を参照してください。たとえば[09]のように記述すると、タブブラウザーであれば常に新しいタブでリンクを開くように指定できます[^10]。
...
<figure>
<figcaption>[09] target属性の記述例</figcaption>
```html
<base target="_blank">
```
</figure>
...
`link`要素の詳細はCHAPTER 3-6<!--(P164)-->で紹介します。
...
[^13]: 文字エンコーディングについてはCHAPTER 2-5を参照してください。
...
CHAPTER 2-5で触れたように、現在のHTML仕様は文字エンコーディングとしてUTF-8を要求するため、`charset`属性の値は、utf-8としなければなりません(大文字小文字は区別しませんので、UTF-8と書いても問題ありません)[^14]。
...
[^16]: `http-equiv`とは、HTTPと等価という意味です。この属性で示される情報は、基本的にHTTPレスポンスヘッダーでも示すことができます。ウェブ上でHTMLコンテンツを提供する場合、HTMLの`meta`要素で指定するよりも、サーバー側の設定によるHTTPレスポンスヘッダーで指定するほうが望ましいでしょう。
...
<figure>
<figcaption>[11] `content`属性を用いた宣言の記述例</figcaption>
```html
...
`link`要素(`<link rel="stylesheet">`)が外部のスタイルシートを参照するのに対し、`style`要素は要素の内容にスタイルを記述します[^26]。`link`要素についてはCHAPTER 3-6を参照してください。
...
`style`の内容モデルはテキストです。このテキストは、CSSの構文として適合するものでなければなりません。
...
[^28]: `style`要素と同じ名前の属性である、`style`属性も存在します。`style`属性はグローバル属性です。CHAPTER 4-1で説明しています。
...
[^29]: `type`属性は廃止されていますが、値が`"text/css"`の場合(大文字小文字は区別しません)のみ、旧式だが適合する機能(Obsolete but conforming)として仕様に適合します。
