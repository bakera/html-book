...
`link`要素でリンクタイプ`preload`を指定すると、現在のページで使用する予定のサブリソースを事前に取得してメモリーに読み込んでおくことが期待できます。`prefetch`は現在のページとは異なる、次のページなどで利用する予定のリソースの想定ですが、`preload`は現在のページで使用する予定のサブリソースの想定です[^48]。
