...
[^1]: URLのスキームや、リンク先リソースのMIMEタイプの値によって異なる挙動になる場合もあります。[CHAPTER 1-6「MIMEタイプ」](1-6.xhtml#mimeタイプ)、[CHAPTER 2-3「サブリソースとURLスキーム」](2-3.xhtml#サブリソースとurlスキーム)を参照してください。
...
後述する[`target`属性<!--(P165)-->](#target属性)について、属性値に`"_blank"`を指定すると、多くの場合、リンク先を新しいブラウザータブで開く挙動になります。
...
[^44]: ブラウジングコンテキストについては、[`target`属性のコラム「ブラウジングコンテキスト」<!--(P166)-->](#コラム-ブラウジングコンテキスト)を参照してください。
