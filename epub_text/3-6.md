...
`hreflang`属性は、*リンク先の言語を示します[^21]。*`lang`属性が現在のコンテンツの言語を指定するのに対し、`hreflang`属性は`href`属性で指定されたリンク先の言語を示します。属性値は言語タグで、これは`lang`属性と同様です。値の詳細は[CHAPTER4-1](4-1.xhtml)の`lang`属性を参照してください。
