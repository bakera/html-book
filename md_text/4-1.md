...
なお、`class`という単語が多くのプログラム言語で予約語となっていることもあり、DOMで`class`属性を参照する場合は`class`プロパティではなく`className`プロパティを利用します。ただし、`className`プロパティは単に属性値にアクセスするため、クラス名が複数指定されている場合、スペースを含む属性値全体を処理することになって煩雑になります。
...
慣習として、JavaScriptから操作することを前提としたクラス名には、`js-`というプレフィクスをつけることがあります。こうすることで、そのクラス名がJavaScriptで操作するためのものであることが明確になります。そして、`js-`で始まるクラス名は、決してCSS側からは参照しないようにします。こうすることで、`js-`で始まるクラス名を変更・削除しても、スタイルに影響が出ないことが保証されます。

#### コラム：状態を伝える属性

要素の状態を`class`属性で表現することもよく行われますが、ARIA属性を利用することで、`class`の指定を省略できることもあります。
...
そして、こうなると実はクラス名は必要なくなります。`class`属性を削除し、CSSのセレクタで`aria-pressed`を参照すればよいのです。
...
このように、ARIA属性をうまく利用すると、意味を伝えつつ、クラス名を減らしてHTMLをスッキリさせられる場合があります。
<!-- /a11y note -->
