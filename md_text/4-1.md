...
言語指定は子孫要素に継承されます。このため、`lang`属性を持たない要素もにも言語の指定は行われていることになります。CSSで特定要素の言語指定を参照する場合は、属性セレクタで`lang`属性の値を見るのではなく、`:lang`擬似クラスを利用します。
...
スクリーンリーダーは、`lang`属性の値によって音声合成エンジンが切り替わることがあります。<!--たとえば、iOSのVoiceOverは`lang`属性を参照して音声合成エンジンを切り替えます。-->適切な言語設定が行われていないと、コンテンツを一切読み上げなかったり、意味不明な読み上げをすることがあります。そのため、`html`要素に`lang`属性を用いてページ全体の言語を適切に指定しておくべきです。[^5]
...
なお、`style`属性で指定した宣言の「詳細度」(specificity)は最高となり、他のあらゆるセレクターよりも優先されます。古いCSS仕様ではIDセレクターと同一の詳細度とされていましたが、現在では`style`属性が最優先です。[^7]。
...
要素の状態を`class`属性で表現するかわりに、ARIA属性を利用することで、`class`の指定を省略できることもあります。
...
正の値を使用すると、見た目とは全く異なるフォーカス移動順を設定することも可能ですが、不用意に使用するとアクセシビリティの問題を起こすことがあります。アクセシビリティ上の注意点も参照してください。
...
スクリーンリーダーは通常、ページの先頭から順に読み上げますが、この属性のある要素が存在すると、その要素から読み上げることがあります。ページ内に読むべきコンテンツが他にある場合は、不用意に利用しない方がよいでしょう。
<!-- /a11y note -->
...
このように、`*`部分の名前はコンテンツ制作者が自由に設定できるのですが、HTMLの属性名に利用できない文字は使えません。
