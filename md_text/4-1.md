...
また、CSSを用いるなどして画面上の表示順序を変更しているために、DOMツリー順が一致していない状況も考えられます。これは、同様にWCAG 2.0の達成基準1.3.2「意味のある順序」も問題を引き起こします[^1]。これに対する対応は`tabindex`属性を指定するのではなく、DOMツリーの順序を変更することを検討します。

[^7]: 参考日本語訳: <https://waic.jp/docs/UNDERSTANDING-WCAG20/navigation-mechanisms-focus-order.html>

[^1]: 参考日本語訳: <https://waic.jp/docs/UNDERSTANDING-WCAG20/content-structure-separation-sequence.html>
...
[^1]: マウスで操作できる一方でキーボードで操作できない場合、一般にWCAG2の観点からは達成基準2.1.1「キーボード」の問題となります。不幸なことに、キーボード操作ができない低品質なウェブサイトは一定数存在します。 <https://waic.jp/docs/UNDERSTANDING-WCAG20/keyboard-operation-keyboard-operable.html>
...
また、CSP (Content-Security-Policy)[^1]においてはインラインのJavaScriptコードとみなされるため、インラインのコードを禁止する設定ではブロックされてしまいます。

[^1]: <https://developer.mozilla.org/ja/docs/Web/HTTP/CSP>
...
HTML仕様にはイベントハンドラーの用語定義[^1]があり、以下のようになっています。
...
[^1]: <https://html.spec.whatwg.org/multipage/webappapis.html#event-handlers>
...
<!-- a11y note -->
`accesskey`属性は、ブラウザーの対応が良好ではなく、OSやブラウザーによって、修飾キーが異なることが知られています[^1]。

[^1]: <https://developer.mozilla.org/ja/docs/Web/HTML/Global_attributes/accesskey>
...
こういった問題のため、通常のウェブサイトやウェブアプリの多くでは、 `accesskey`属性を使用しないよう一般にアドバイスされます[^1]。

[^1]: WebAIM: Keyboard Accessibility - Accesskey <https://webaim.org/techniques/keyboard/accesskey>
...
この属性が指定されると、ブラウザーは要素をレンダリングしなくなります。これは`hidden`属性を指定すると、その要素は、ブラウザーで視覚的に非表示になるだけでなく、スクリーンリーダーでも読み上げられなくなるという挙動になります。CSSの`display:none`の指定と同じ挙動だと考えるとよいでしょう。`hidden`属性が指定された要素に子孫要素がある場合、子孫要素もまるごと非表示になります。
...
ブラウザーは通常、`hidden`属性の指定された要素に対して、CSSの`display: none`を適用します。しかし、このスタイル指定の優先度は高くないため[^8] [^9]、CSSで明示的に`display`プロパティを指定すると、`hidden`属性を指定しても非表示にならないことがあります。

[^8]: ユーザーエージェントスタイルシートとして、`!important`指定のないスタイルを適用することになっています。仕様の15.3.1 Hidden elementsを参照してください。<https://html.spec.whatwg.org/multipage/rendering.html#hidden-elements>

[^9] スタイルシートのカスケーディング順序はCSS Cascading and Inheritance仕様§6. Cascadingで定義されているとおりです。 <https://www.w3.org/TR/css-cascade/#cascade-sort>
...
このようにJavaScriptのコードで面倒なことになるため、`*`部分の名前を記号や数字で始めるのは避けるのが無難と言えます。
