...
この属性が指定されると、ブラウザーは要素をレンダリングしなくなります。端的に言えば要素を非表示にする効果がありますが、視覚だけでなく、あらゆるプレゼンテーションから要素を隠します。つまり、`hidden`属性を指定した要素は、ビジュアルブラウザで見えないだけでなく、スクリーンリーダーでも読まれなくなります。CSSの`display:none`の指定と同じ挙動だと考えるとよいでしょう。`hidden`属性が指定された要素に子孫要素がある場合、子孫要素もまるごと非表示になります。
...
また、WAI-ARIAの`aria-labelledby`属性などでは参照することが可能です。
...
カスタムデータ属性を利用すると、任意の要素に任意の属性値を指定することができます。この属性は特別な効果を持たず、ユーザーに対して表示されることもありません。主にJavaScriptから参照して利用します。

カスタムデータ属性の属性名は、`data-`から始まり、その後に1文字以上の任意の名前をつけることができます。JavaScriptからカスタムデータ属性を利用する際には、HtmlElementのdatasetプロパティを通じてアクセスすることができます。属性名から先頭の`data-`を除去したものがプロパティ名となります。
...
- XML構文の場合には、ASCIIアルファベットの大文字 (A-Z) を使用することができません。
...
混乱のもとになりますので、基本的にはASCIIアルファベットの大文字は使わない方が良いでしょう。
