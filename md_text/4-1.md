...
グローバル属性の中には、HTML仕様では規定されていないものもあります。WAI-ARIA仕様で規定されている`role`属性や`aria-*`属性や、XML仕様で規定されている`xml:lang`や`xml:space`など[^1]が典型例です。
...
[^3] アクセシブルな名前は、HTML Accessibility API MappingsのAccessible Name and Description Computationに基づいて決定されます。 <https://w3c.github.io/html-aam/#accessible-name-and-description-computation>
<!-- /a11y note -->
...
アクセシビリティ上の重要な点として、スクリーンリーダーの音声合成エンジンに影響することが挙げられます。iOSのVoiceOverは`lang`属性を参照して音声合成エンジンを切り替えます。適切な言語設定が行われていないとコンテンツを一切読み上げないことがあるため、アクセシビリティの観点からも、`html`要素に`lang`属性を用いてページ全体の言語を適切に指定しておくべきです。[^4]
<!-- /a11y note -->
...
<!-- `application/xhtml+xml`で`xml:lang`が優先されるとかは？ -->
...
要素に`style`属性を指定すると、その要素に対してCSSのスタイルを設定できます。属性値にはCSSの「宣言」(declaration)を記述します。セミコロンで区切って複数の宣言を記述することもできます。[^5]
...
「マイクロフォーマット」(Microformats)と呼ばれる仕様では、（HTML4に対して）要素や属性を追加することなく、既存のHTMLの`class`属性や`rel`属性などを利用して、その要素の意味を表すという試みです。以下の例は、マイクロフォーマットの仕様のひとつであるh-card[^7]の例です。
