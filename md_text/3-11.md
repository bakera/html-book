...
`script`要素を利用すると、HTML内に動的なスクリプトを埋め込んだり、外部のスクリプトを参照したりできます。

他のHTML要素とは異なり、ユーザーに何かを提示するものではありません。`script`要素はテキストをマークアップするものではなく、特定のセマンティクスを持ちません。ARIAロールとも無縁です。
<!--
 spec: 4.12.1
