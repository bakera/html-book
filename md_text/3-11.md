...
### crossorigin属性
`crossorigin`属性を指定すると、`src`属性で指定されたリソースをクロスオリジンで参照する際のCORS (Cross origin Resource Sharing) の挙動を制御することができます。CORSの詳細についてはFetch Standardを参照してください。[^5]
...
`integrity`属性を使用すると、`src`属性で指定した外部スクリプトの改竄を検出し、読み込みをブロックすることができます。この仕組みは「サブリソース完全性」(Subresource Integrity) と呼ばれます。詳しい仕様は、W3Cエディターズドラフト文書の"Subresource Integrity"を参照してください。[^6]
