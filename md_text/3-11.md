...
`crossorigin`属性を指定すると、`src`属性で指定されたリソースをクロスオリジンで参照する場合のCORS (Cross origin Resource Sharing)の挙動を制御できます。CORSの詳細についてはFetch Standard[^5]を参照してください。
...
-->
クラシックスクリプトの場合、`crossorigin`属性が指定されていないと、リクエストの「モード」(mode)は"no-cors"となります。これはCORSによるリクエストではなく、単なるGETリクエストによる参照です。この場合、リクエストがエラーとなっても最小限の情報しか得られず、詳細がわからないことがあります。

`crossorigin`属性が指定されていると、モードが"cors"となり、CORSによるリクエストを行います。この場合、リクエストに`Origin:`などのCORS関連のヘッダーが追加されて送信されるようになります。また、エラー時には`window.onerror`で詳細なエラー情報が取得できます。

`crossorigin`属性の値が空か、あるいは"anonymous"が指定されている場合は、「クレデンシャルモード」(credentials mode)が"same-origin"となります。この場合、HTTP認証やCookieなどのクレデンシャル情報は、同一オリジンに対してのみ送信されます。クロスオリジンの場合には送信されません。

`crossorigin`属性に"use-credentials"を指定するとクレデンシャルモードが"include"になります。この場合、Cookieなどのクレデンシャル情報が常に送信されるようになります。
...
以上はクラシックスクリプトの場合ですが、モジュールスクリプトの場合、`crossorigin`属性の指定にかかわらず、常にモードが"cors"となります。クレデンシャルモードについては`crossorigin`属性の影響を受け、"use-credentials"を指定するとクレデンシャル情報が常に送信されるようになります。
...
-->
`integrity`属性を使用すると、`src`属性で指定した外部スクリプトの改竄を検出し、読み込みをブロックできます。この仕組みは「サブリソース完全性」(Subresource Integrity) と呼ばれます。詳しい仕様は、W3Cエディターズドラフト文書の"Subresource Integrity"[^6]を参照してください。

[^6]: <https://w3c.github.io/webappsec-subresource-integrity/>

`script`要素の`src`属性で外部リソースを指定すると、そのリソースがスクリプトとして実行されます。外部スクリプトをホストしているサイトが攻撃を受け、スクリプトが改竄された場合、読み込んでいる側はその影響を受けてしまいます。

このようなリスクに対応するため、リソースを読み込む側は、`script`要素の`integrity`属性にリソースのハッシュ値を指定できます。あらかじめ正しいリソースを取得してハッシュ値を得ておき、ブラウザーがリソースを読み込むときにハッシュ値が一致するか検証することで、改竄を検出できます。
...
`referrerpolicy`属性を使用すると、スクリプトを参照する場合の「リファラポリシー」(Referrer Policy)を指定できます。外部リソースの取得時に"Referer"を送りたくない場合などに利用できます。詳しい仕様は、W3Cエディターズドラフト文書の"Subresource Integrity"[^7]を参照してください。

[^7]: <https://w3c.github.io/webappsec-referrer-policy/>

属性名の前半の綴りは"referrer"であり、"referer"ではないことに注意してください。HTTPヘッダーの"Referer"は元々スペルミスだったものを直さずに使っていますが、"Referrer-Policy"ヘッダーは正しいスペルを採用しています。この属性名も正しいスペルを使用しています。
