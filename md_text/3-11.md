...
`script`要素の内容モデルはきわめて複雑です。<!--HTML仕様の"13.2.5 Tokenization"にある"Script data..."で始まる項目は全て`script`要素の内容を解析するためのものです。HTMLの全ての字句を解析するのに必要なステートが全部で80種類、そのうちの17種類がスクリプト関連ですから2割以上がスクリプトのために費やされています。ちなみにJavaScriptの構文解析は全くしていません。HTMLとしての解析だけに限った話でこれです。-->
まず、有効な`src`属性が指定されている場合には、意味のある内容を含めることはできません。内容は空であるか、または、空白類文字、改行、JavaScriptのコメントと解釈される値のみを含めることが可能です。

有効な`src`属性が指定されていない場合には、任意の内容を含めることができます。ただし、以下の文字列が出現した場合には特殊な解釈が行われるため、不用意に含めないようにしてください。

- `<!--`
- `-->`
- `<script`

また、`</script`という文字列は原則として`script`要素の終了タグとみなされるため、要素内に含めることはできません (例外もありますが、ここでは割愛します)。

以下は仕様で挙げられている、意図どおりに解析されない例です。
...
上記には `<!--`および`<script>`という文字列が含まれていますが、これらは`script`要素の内容を解析する上で特別扱いされます。その結果、上記の例は正しく解析されず、末尾の`</script>`がscript要素の終了タグとみなされません。
<!--
...
最後まで解析しても状態は"Script data escaped state"であり、`script`要素内の解析モードのままです。つまり最後の`</script>`は終了タグとして認識されず、`script`要素の内容とみなされていることになります。
-->
