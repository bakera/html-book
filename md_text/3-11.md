...
HTML仕様自身はスクリプト言語の実装を強制しませんが、JavaScriptに依存して記述されています。技術的にブラウザーはJavaScriptを実装しています。
...
`type`属性を指定すると、この要素のタイプを規定できます。

`script`要素には大きく分けて3種類の用途があり、それぞれ「クラシックスクリプト」(classic scripts)、「モジュールスクリプト」(module scripts)、「データブロック」(data blocks)と呼ばれます。

#### クラシックスクリプト

`type`属性にJavaScriptを意味するMIME typeの値[^1]、たとえば"application/javascript"[^2]や"text/javascript"などを指定すると、クラシックスクリプトとして扱われます。`script`要素の内容、もしくは`src`属性で指定したリソースはJavaScriptのスクリプトとして解釈され、実行されます。
<!--
 the JavaScript Script top-level production. 
-->

[^1]: クラシックスクリプトとみなされる値の正確な一覧は、MIME Sniffing Living Standardで確認できます。<https://mimesniff.spec.whatwg.org/#javascript-mime-type>

[^2]: RFC 4329で登録されたMIMEタイプですが、ECMAScript Media Types UpdatesというI-Dで廃止扱いとすることが提案されています。<https://datatracker.ietf.org/doc/html/draft-ietf-dispatch-javascript-mjs>
...
<!-- 互換性メモ -->
<!-- マニアックな話なのでコラム扱いでも良い -->
また、古くに書かれたHTMLでは、`type`属性の代わりに`language`属性が指定されていたこともありました。現在のHTMLでは`language`属性は廃止されており、指定すると文法違反になります。
...
-->
モジュールはECMAScript 2015で新たに導入されたもので、スクリプトをモジュールに分割して扱えるようにしたものです。モジュールでは`export`で必要な機能を公開し、それを`import`で読み込んで利用できます。詳細については本書では説明しません。JavaScriptのチュートリアルを参照ください[^1]。

[^1]: MDN <https://developer.mozilla.org/ja/docs/Web/JavaScript/Guide/Modules>や JSPrimer <https://jsprimer.net/basic/module/>が参考になるでしょう。
...
[^2] 正確には、クラシックスクリプトにもモジュールスクリプトにも当てはまらない値はすべてデータブロックとみなされます。しかし仕様では、`type`属性はMIME typeでなければならないとされています。これは、将来の仕様で`type`属性に"module"以外のキーワードが使えるようになったときに、誤動作しないようにするためです。

ブラウザーは、データブロックに対して何の処理も行いません。内容がスクリプトとして実行されることもありませんし、ウェブページに表示されることもありません。`script`要素の属性のうち、`type`以外の属性は指定できず、すべて無視されます（`id`などのグローバル属性は有効です）。[^3]
...
JSON (JavaScript Object Notation)[^1]データを埋め込むこともできます。以下は、JSON-LD (JSON for Linked Data)[^2]によるメタデータ[^3]を埋め込んでいる例です。

[^1]: ジェイソンと発音。RFC 8259/ECMA-404 2nd editionで定義される、JavaScript構造データオブジェクトの表記法をベースにしたフォーマットです。<https://datatracker.ietf.org/doc/html/rfc8259> <https://www.ecma-international.org/publications-and-standards/standards/ecma-404/>

[^2]: JSON-LD 1.1としてW3Cから勧告されています。 <https://www.w3.org/TR/json-ld11/>

[^3]: RDFaやMicrodataが類似のメタデータ表現として知られています。
...
[^4]: JavaScriptのコメントと改行、空白類文字のみが許されます。詳細は仕様を参照ください。<https://html.spec.whatwg.org/multipage/scripting.html#inline-documentation-for-external-scripts>
...
ここで`src`属性から少し離れて、一般的な話をしたいと思います。

ウェブにおけるセキュリティの原則として、「同一オリジンポリシー」（Same-Origin Policy）と呼ばれる重要な概念があります[^1][^2]。基本的な考え方は以下のとおりです。

- URLによって信頼（trust）できるかを指定する
- URL単位では実用上不便であるので、「オリジン」（origin）[^3]という単位で管理する
  - オリジンは端的には、スキーム、ホスト、ポートの組み合わせとなる[^4]
...
しかし、クロスオリジンでアクセス権限は制約されたままですと都合が悪いと言えます。そこで、ブラウザーに対してアクセス権限を与えるように追加のHTTPヘッダーを利用して指示する仕組みとして、CORS (Cross-Origin Resource Sharing)[^5]と呼ばれるものがあります。

[^1]: この概念自体は、RFC 6454でまとめられたものです。<https://datatracker.ietf.org/doc/html/rfc6454> しかし、[^3]のように他の仕様で上書きされているものがあります。

[^2]: 詳しくはMDNを参照ください。 <https://developer.mozilla.org/ja/docs/Web/Security/Same-origin_policy>

[^3]: 生成元と訳されることもあります。

[^4]: 現在はHTML Standardで定義されています。 <https://html.spec.whatwg.org/multipage/origin.html#origin>

[^5]: CORSの詳しい説明はMDNを参照ください。 <https://developer.mozilla.org/ja/docs/Web/HTTP/CORS>

<!-- #### セキュリティ上の注意点: スクリプトのクロスオリジン参照 

`src`属性には任意のURLを指定できます。ウェブページと異なるドメインのURL、すなわち、「オリジン」(origin)[^1]が異なるURLを指定することも可能です。
...
スクリプトの処理内容によっては、単純に`defer`や`async`を指定するとうまく動作しないことがあります。たとえば、`document.write()`を実行しているスクリプトを`defer`や`async`で実行するとエラーになります。実際にChromeではコンソールに次のエラーメッセージが出力されます。

> Failed to execute 'write' on 'Document': It isn't possible to write into a document from an asynchronously-loaded external script unless it is explicitly opened.

`document.write()`以外にも、HTMLのDOMツリーを参照するような処理が動作しなくなることがあります。`DOMContentLoaded`イベントをトリガーにして動作させるなど、スクリプト側で遅延実行を想定した実装をしておく必要があります。
...
`crossorigin`属性を指定すると、`src`属性で指定されたリソースをクロスオリジンで参照する場合のCORSの挙動を制御できます。CORSはFetch Standard[^5]で定義されています。
...
-->
クラシックスクリプトにおいて、`crossorigin`属性が指定されていない場合、Fetch仕様で定められたリクエストの「モード」(mode)は"no-cors"となります。これはCORSによるリクエストではなく、単なるGETリクエストによる参照です。この場合、リクエストがエラーとなっても最小限の情報しか得られず、詳細がわからないことがあります。

`crossorigin`属性が指定されている場合、モードは"cors"となり、CORSによるリクエストを行います。この場合、リクエストに`Origin:`などのCORS関連のHTTPヘッダーが追加されて送信されるようになります。また、エラー時には`window.onerror`イベントハンドラーで詳細なエラー情報が取得できます。
...
-->
`integrity`属性を使用すると、`src`属性で指定した外部スクリプトが改竄されていないかをブラウザーが検出できます。この仕組みは「サブリソース完全性」(Subresource Integrity) と呼ばれます。仕様としてはSubresource Integrity[^6]で規定されています。
...
`referrerpolicy`属性を使用すると、スクリプトを参照する場合の「リファラポリシー」(Referrer Policy)を指定できます。外部リソースの取得時に"Referer"を送りたくない場合などに利用できます。仕様としては、Referrer Policy[^7]で規定されています。

[^7]: <https://w3c.github.io/webappsec-referrer-policy/>
...
`script`要素の内容モデルは複雑です。<!--HTML仕様の"13.2.5 Tokenization"にある"Script data..."で始まる項目は全て`script`要素の内容を解析するためのものです。HTMLの全ての字句を解析するのに必要なステートが全部で80種類、そのうちの17種類がスクリプト関連ですから2割以上がスクリプトのために費やされています。ちなみにJavaScriptの構文解析は全くしていません。HTMLとしての解析だけに限った話でこれです。-->
まず、有効な`src`属性が指定されている場合には、意味のある内容を含めることはできません。内容は空であるか、または、空白類文字、改行、およびJavaScriptのコメントと解釈される値のみを含めることが可能です。`src`属性の項も参照してください。

有効な`src`属性が指定されていない場合は任意の内容を含めることができます。内容はそのままスクリプトエンジンに渡され、JavaScriptとして解釈されます （`script`要素がデータブロックである場合は、何もしません）。

ただし、内容に含められないものもあります。

#### `script`要素の内容の指針

後述するように、`script`要素の内容の挙動は、過去との互換性の都合もあって非常に複雑なものとなっています。以下のようにするとよいでしょう。
...
HTML構文の場合[^7]、`script`要素内に`<!--`という文字列があっても、HTMLのコメントとしては扱わず、内容の一部として扱われます。`<!--`や`-->`はそのままスクリプトエンジンに渡され、スクリプトエンジンの側でJavaScriptのコメントとして扱われます[^8]。

[^7]: XML構文の場合にはXMLのコメントみなされ、内容がスクリプトエンジンに渡りません。

[^8]: ECMAScript 2015では、互換性のためのHTML-likeコメントが正式に仕様に追加され、HTMLのコメント区切り子（`<!--` および `-->`）をECMAScriptの1行コメントして解釈するよう定めています。つまり、これらは`//`と同じ効力を持つことになります。<https://262.ecma-international.org/6.0/#sec-html-like-comments>
...
`script`要素内に`<!--`が出現した場合に、さらに`<script`が出現していると、対応する`</script`が出現しても終了タグとみなさない処理が行われます[^9]。そのため、上記の例はスクリプト中に`</script>`を含んでいるにもかかわらず、問題なく動作します。
...
[^9]: HTMLパーサーの字句解析機の挙動としては、`<!--`が出現すると"Script data escaped state"という状態に遷移し、そこで`<script`が出現すると"Script data double escaped state"に遷移します。このときに`</script`が出現しても終了タグとみなさず、単に"Script data escaped state"に戻ります。
