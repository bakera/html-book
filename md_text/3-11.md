...
これは、先の節で挙げたスクリプトエラーとなる例に対して、`<!--`と`-->`を追加しただけのものです。同様にエラーになると予想されるかもしれませんが、これはなんと、問題なく動作します。最初にした`</script>`という文字列は、`script`要素の終了タグとはみなされないのです。

`script`要素内に`<!--`が出現した場合に、さらに`<script`が出現していると、対応する`</script`が出現しても終了タグとみなさない処理が行われます。そのため、上記の例はスクリプト中に`</script>`を含んでいるにもかかわらず、問題なく動作します。
