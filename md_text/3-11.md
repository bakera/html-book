...
現在のHTMLでは、外部JavaScriptファイルの文字符号化方式はHTTP応答ヘッダの`charset`パラメータに依存し、`charset`パラメータがない場合はUTF-8とみなされます[^6]。`charset`属性は基本的に無視されますので、指定しないようにしましょう。

[^6]: RFC 4329 Scripting Media Types <https://www.rfc-editor.org/rfc/rfc4329.html#section-4.2>
<!-- /互換性メモ -->
...
モジュールはECMAScript 2015で新たに導入されたもので、スクリプトをモジュールに分割して扱えるようにしたものです。モジュールでは`export`で必要な機能を公開し、それを`import`で読み込んで利用できます。詳細については本書では説明しません。JavaScriptのチュートリアルを参照ください[^7]。

[^7]: MDN <https://developer.mozilla.org/ja/docs/Web/JavaScript/Guide/Modules>や JSPrimer <https://jsprimer.net/basic/module/>が参考になるでしょう。
...
`type`属性が妥当なMIME typeであり、かつJavaScriptとみなされないものである場合、`script`要素はデータブロックとして扱われます[^8]。

[^8]: 正確には、クラシックスクリプトにもモジュールスクリプトにも当てはまらない値はすべてデータブロックとみなされます。しかし仕様では、`type`属性はMIME typeでなければならないとされています。これは、将来の仕様で`type`属性に"module"以外のキーワードが使えるようになったときに、誤動作しないようにするためです。

ブラウザーは、データブロックに対して何の処理も行いません。内容がスクリプトとして実行されることもありませんし、ウェブページに表示されることもありません。`script`要素の属性のうち、`type`以外の属性は指定できず、すべて無視されます（`id`などのグローバル属性は有効です）。[^9]

[^9]: データブロックに指定できない属性は次の7つです: "src"、"async"、"nomodule"、"defer"、"crossorigin"、"integrity"、"referrerpolicy"。
...
JSON (JavaScript Object Notation)[^10]データを埋め込むこともできます。以下は、JSON-LD (JSON for Linked Data)[^11]によるメタデータ[^12]を埋め込んでいる例です。

[^10]: ジェイソンと発音。RFC 8259/ECMA-404 2nd editionで定義される、JavaScript構造データオブジェクトの表記法をベースにしたフォーマットです。<https://datatracker.ietf.org/doc/html/rfc8259> <https://www.ecma-international.org/publications-and-standards/standards/ecma-404/>

[^11]: JSON-LD 1.1としてW3Cから勧告されています。 <https://www.w3.org/TR/json-ld11/>

[^12]: RDFaやMicrodataが類似のメタデータ表現として知られています。総称して構造化データとも呼ばれることがあります。
...
`src`属性を指定している場合、`script`要素の内容に意味のあるスクリプトを書くことはできません[^13]。内容があるとエラーとなりますが、その場合は内容は無視され、`src`属性の指定が優先されます。
...
ウェブにおけるセキュリティの原則として、「同一オリジンポリシー」（Same-Origin Policy）と呼ばれる重要な概念があります[^14][^15]。基本的な考え方は以下のとおりです。

- URLによって信頼（trust）できるかを指定する
- URL単位では実用上不便であるので、「オリジン」（origin）[^16]という単位で管理する
  - オリジンは端的には、スキーム、ホスト、ポートの組み合わせとなる[^17]
...
しかし、クロスオリジンでアクセス権限は制約されたままですと都合が悪い場合があります。そこで、ブラウザーに対してアクセス権限を与えるように追加のHTTPヘッダーを利用して指示する、CORS (Cross-Origin Resource Sharing)[^18]と呼ばれる仕組みを利用して、アクセスを許可します。

[^14]: この概念自体は、RFC 6454でまとめられたものです。<https://datatracker.ietf.org/doc/html/rfc6454> しかし、[^16]のように他の仕様で内容が上書きされているものがあります。

[^15]: 詳しくはMDNを参照ください。 <https://developer.mozilla.org/ja/docs/Web/Security/Same-origin_policy>

[^16]: 生成元と訳されることもあります。

[^17]: 現在はHTML Standardで定義されています。 <https://html.spec.whatwg.org/multipage/origin.html#origin>

[^18]: CORSの詳しい説明はMDNを参照ください。 <https://developer.mozilla.org/ja/docs/Web/HTTP/CORS>
...
`crossorigin`属性を指定すると、`src`属性で指定されたリソースをクロスオリジンで参照する場合のCORSの挙動を制御できます。CORSはFetch Standard[^19]で定義されています。
...
-->
`integrity`属性を使用すると、`src`属性で指定した外部スクリプトが改竄されていないかをブラウザーが検出できます。この仕組みは「サブリソース完全性」(Subresource Integrity) と呼ばれます。仕様としてはSubresource Integrity[^20]で規定されています。

[^20]: <https://w3c.github.io/webappsec-subresource-integrity/>
...
このようなリスクに対応するために、リソースを読み込む側があらかじめ正しいリソースのハッシュ値を得ておき、ブラウザーがそのリソースを読み込むときにハッシュ値が一致するか検証することで、改竄を検出という方法が考えられます。`integrity`属性はこのハッシュによるチェックを行います。
...
`referrerpolicy`属性を使用すると、スクリプトを参照する場合の「リファラポリシー」(Referrer Policy)を指定できます。外部リソースの取得時に"Referer"を送りたくない場合などに利用できます。仕様としては、Referrer Policy[^21]で規定されています。

[^21]: <https://w3c.github.io/webappsec-referrer-policy/>
...
`script`要素の内容モデルは複雑です[^22]。まず、有効な`src`属性が指定されている場合には、意味のある内容を含めることはできません。内容は空であるか、または、空白類文字、改行、およびJavaScriptのコメントと解釈される値のみを含めることが可能です。`src`属性の項も参照してください。
...
[^22]: HTML仕様の"13.2.5 Tokenization"にある"Script data..."で始まる項目は全て`script`要素の内容を解析するためのものです。HTMLのすべての字句を解析するのに必要なステートが全部で80種類であり、そのうちの17種類がスクリプト関連ですから、2割以上がスクリプトのために費やされています。
...
HTML構文の場合[^23]、`script`要素内に`<!--`という文字列があっても、HTMLのコメントとしては扱わず、内容の一部として扱われます。`<!--`や`-->`はそのままスクリプトエンジンに渡され、スクリプトエンジンの側でJavaScriptのコメントとして扱われます[^24]。

[^23]: XML構文の場合にはXMLのコメントみなされ、内容がスクリプトエンジンに渡りません。

[^24]: ECMAScript 2015では、互換性のためのHTML-likeコメントが正式に仕様に追加され、HTMLのコメント区切り子（`<!--` および `-->`）をECMAScriptの1行コメントして解釈するよう定めています。つまり、これらは`//`と同じ効力を持つことになります。<https://262.ecma-international.org/6.0/#sec-html-like-comments>
...
`script`要素内に`<!--`が出現した場合に、さらに`<script`が出現していると、対応する`</script`が出現しても終了タグとみなさない処理が行われます[^25]。そのため、上記の例はスクリプト中に`</script>`を含んでいるにもかかわらず、問題なく動作します。
...
[^25]: HTMLパーサーの字句解析機の挙動としては、`<!--`が出現すると"Script data escaped state"という状態に遷移し、そこで`<script`が出現すると"Script data double escaped state"に遷移します。このときに`</script`が出現しても終了タグとみなさず、単に"Script data escaped state"に戻ります。
...
`canvas`要素の内容は、`canvas`要素が利用できない場合のフォールバックコンテンツてして機能します。`canvas`要素が画像として使用されている場合は、代替となる`img`要素などを入れておくとよいでしょう。より複雑な機能を提供している場合は、可能な限りその機能を代替できる内容を入れておくべきです。
<!-- /内容モデル -->
...
Web ComponentsはJavaScriptでの操作を行うことになりますので、JavaScriptに関係する内容については他に譲るものとして[^26]、HTML要素として存在する、`template`要素、`slot`要素、カスタム要素について本書では簡単に触れていくことにします。

[^26]: 例えばMDNのWeb Componentsにまとまった情報が記載されています。 <https://developer.mozilla.org/docs/Web/Web_Components>
...
JavaScriptを用いて、カスタム要素自身とその動作を定義していくことになりますが、本書の範囲を超えますので、詳細についてはMDN[^27]やGoogle Developers[^28]の記事などを参照してください。

[^27]: Using custom elements - Web Components | MDN <https://developer.mozilla.org/docs/Web/Web_Components/Using_custom_elements>

[^28]: カスタム要素 v1: 再利用可能なウェブ コンポーネント  |  Web  |  Google Developers <https://developers.google.com/web/fundamentals/web-components/customelements?hl=ja>
