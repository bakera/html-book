...
同様に、`script`要素の`charset`属性も廃止されています。これは、外部JavaScriptファイルの文字エンコーディング方式を指定するものでした。現在では、`charset`属性の値が"utf-8"の時 (大文字小文字は区別しません) のみ、「旧式だが適合する機能」(Obsolete but conforming)として扱われ、仕様に適合します。<!--理由は不明。https://developer.mozilla.org/ja/docs/Web/HTML/Element/Script#deprecated_attributes にはHTML文書自体のcharsetと同じでありHTML文書がUTF-8だからという意味のことが書いてあるが、上記のとおり外部JSの文字符号化方式を指定するものなので、これは誤り。もしかするとLSのこの箇所を書いた人自身が勘違いしているのかも?-->

現在のHTMLでは、外部JavaScriptファイルの文字エンコーディング方式はHTTPレスポンスヘッダーの`charset`パラメータに依存し、`charset`パラメータがない場合はUTF-8とみなされます[^12]。`charset`属性は基本的に無視されますので、指定しないようにします。
