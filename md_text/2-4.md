...
ここでは`ul`の直下に`ins`要素を入れようとしていますが、`ul`の直接の子要素になれるのは`li`だけです。`ins`要素自体は、他の多くの要素と同様に"Flow"と"Phrasing"に属しているため、それらが許される場所でしか使えません。`li`の内容モデルは"Flow"であるため、`ins`要素を`li`の子にすることは可能です。先の例は以下のように書き直すことができます。
