...
内容モデルが"transparent"とされている要素もあります。transparentとは、透明あるいは透過的などと訳される単語です。

transparentに該当する要素は、`a`要素や`ins`要素、`del`要素などです。これらの要素に含めることができる内容は、親要素の内容モデルを受け継ぎます。どの要素が親になっているかによって、子にできる内容が変化するのです。
...
transparentは、transparentとされている要素の開始タグ・終了タグを取り除いても問題がないようにマークアップする必要がある、とおおまかには考えればよいでしょう。つまり、その意味では透明であると捉えることができます。
