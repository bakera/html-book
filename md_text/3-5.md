...
`target`属性は、もともとはフレームセット文書を制御する属性として定義されていたものです。そのため、フレームセット文書を定義する`frameset`要素と`frame`要素の廃止に伴って廃止される可能性がありましたが、インラインフレームを表す`iframe`要素は現在でも使われており、また、リンク先を別タブで開く際の制御にも用いられています。こういった事情から、`target`属性は現在でも引き続き利用可能となっています。
<!-- /互換性メモ -->
