...
`a`要素は、「アンカー」(anchor)を表現する要素です。`href`属性を指定することで、ハイパーリンクとして機能します。
...
多くの場合は`href`属性を指定して利用します。`href`属性を指定した場合、この要素はハイパーリンクとして機能します。`href`属性を含む、`a`要素の属性の詳細については、「リンク関連要素の属性」を参照してください。
...
後述する`target`属性について、属性値に"`_blank`"を指定すると、多くの場合、リンク先を別のブラウザータブで開く挙動になります。
...
ウェブページの冒頭に大きなヘッダーがある場合、スクリーンリーダーのユーザーやキーボードユーザーは、メインコンテンツに辿り着くまでの多大な労力が必要になることがあります。そのため、WCAG 2.1達成基準2.4.1ブロックスキップ[^23]では、ブロックをスキップする機能を提供するように求めています。
...
`link`要素では、`href`属性は必須です。`a`要素についてはたいていの場合、`href`属性を利用します。

`href`属性には、リンクの対象リソースのURLを指定します。指定できる値は、「潜在的にスペースで囲まれた妥当なURL」(valid URL potentially surrounded by spaces)です。先頭と末尾のASCII空白文字は、ブラウザーがHTMLを解析するときに取り除かれます。たとえば、以下の2つの`a`要素は同じ意味になります。
...
`link`要素では多くの場合、`href`属性とともに`rel`属性を指定します。
