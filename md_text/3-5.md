...
`rel`属性は、リンクの種類、リンク先のこの文書との関連性を示します。属性値は「リンクタイプ」(link types)で、空白で区切って複数指定することもできます。リンクタイプについては、後述の「リンクタイプ」で詳しく説明します。
...
リンクタイプは、現在の文書とリンク先のリソースとの関係を表すキーワードで、HTML仕様の4.6.6 Link types[^7]にて定義されています。また、拡張リンクタイプ[^8]として誰でも登録することができます。

リンクタイプのキーワードは大文字小文字を区別しません。たとえば、"stylesheet"と"StyleSheet"と"STYLESHEET"は同じ意味になります。

[^7]: <https://html.spec.whatwg.org/multipage/links.html#linkTypes>

[^8]: https://microformats.org/wiki/existing-rel-values#HTML5_link_type_extensions
