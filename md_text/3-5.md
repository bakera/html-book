...
リンクタイプ`author`は、リンク先がコンテンツ制作者に関連する情報であることを示します。`mailto:`スキームのURLを使用して、コンテンツ制作者の電子メールアドレスを示すことが一般的です。`link`要素で用いた場合、このHTML文書全体のコンテンツ制作者を指します。

```html
<link rel="author" href="mailto:me@example.com">
```

このリンクタイプを`a`要素、`area`要素で用いた場合は、最も近い祖先の`article`要素のコンテンツ制作者を指すものと解釈されます。`article`要素の外で用いられているときは、HTML文書全体のコンテンツ制作者を指します。

古いHTMLでは同じ意味で`rev=made`が使われていました。現在のHTMLでは`rev`属性は廃止されていますが、互換性のために、ブラウザーは`rev=made`を`rel=author`と同じ意味に解釈します。

古いブラウザー<!-- lynxとか -->の中には、これを解釈し、文書のコンテンツ制作者に即座に電子メールを送信できる機能を持つものがありました。現在のブラウザーはそのような機能をほとんどサポートしていませんので、無理に指定する必要はないでしょう。
