...
仕様には説明はありませんが、Googleでは、モバイル向けページ（`https://m.example.com/1`）とパソコン向けページ（`https://www.example.com/1`）で別々のURLがある場合、それぞれのページに次のようなアノテーションを指定することで、2つのURLの関係を認識することが謳われています。
...
```
このように記述することで、クエリー部を無視したURLに統一させることが見込まれます。
...
これは信頼できないリンクを開く場合、 Window.openerプロパティでリンク元の文書を変更できないようにするために特に役に立ちます。ただし、 Referer HTTP ヘッダーは（noreferrer を使用しない限り）提供します。
...
```
<a href="http://example.com/" rel="noreferrer">リファラーを送信しないリンク</a>
```
...
```
`media`属性の値は、[メディアクエリー仕様](https://drafts.csswg.org/mediaqueries/)に定められた（複雑な）値を取ることができます。古いHTMLで定義されていた、一部のメディアタイプはもはや非推奨です。
<!-- 何かもう少し例を挙げる？ -->

古いHTMLでは`charset`属性で文字エンコーディングを指定できましたが、もはや`link`要素に対してこの属性は指定してはなりません。

また、`alternate`と組み合わせることで、機能が変化します。`alternate`も併せて参照ください。
