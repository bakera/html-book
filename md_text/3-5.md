...
ユーザーがハイパーリンクを発見したとき、そのリンクを辿るかどうか判断するためには、リンク先の内容を推測する必要があります。WCAG 2.1達成基準2.4.4リンクの目的 (コンテキスト内)[^3]では、前後の文脈も込みでリンクの目的を理解できるようにすることを求めています。
...
この例では前後の文脈もなく、リンク先がどのような内容か推測できません。以下のように、リンク先に関する情報が`a`要素の外に書かれていることもあります。
...
この例では、`a`要素の直前のテキストを読むことでリンク先の内容がわかり、前後の文脈からリンク先を推測できることになります。そのため、最低限のアクセシビリティは担保できているといえます。

より高度なアクセシビリティ基準として、WCAG 2.1達成基準2.4.9リンクの目的 (リンクのみ)[^4]があります。こちらでは、前後の文脈に頼らず、リンクテキストのみでリンク先を理解できるようにすることを求めています。スクリーンリーダーをはじめとする多くの支援技術は、次のリンクに飛ぶ機能やリンクだけを読み上げる機能を持ち、前後の文とは切り離されてリンクテキストだけが読まれることがあるためです。
