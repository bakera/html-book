...
<!-- セキュリティ上の注意点 -->
新しいブラウジングコンテキストで他のHTMLを開いた場合、JavaScriptの`window.opener`を参照することで元のブラウジングコンテキストを参照できます。古いブラウザーでは、このブラウジングコンテキストの参照がクロスドメインで行える場合があり、セキュリティ上の重大な注意点とされていました。現在のブラウザーではクロスドメイン参照はできなくなっています。
<!-- /セキュリティ上の注意点 -->
