...
実行結果の出力先には`span`など他の要素を利用することもできますが、`output`要素にはスクリプトによる動的処理に適した以下のような機能があります。

- 自身をフォームや入力欄と関連づける機能がある
...
この例では、`for`属性で、この出力に関連する入力欄を指定しています。属性値には対応する要素のIDをスペース区切りで記述します。

`name`属性を指定でき、スクリプトから参照する際に利用できます。もちろん`id`属性を利用しても構いませんが、`name`属性は重複しても問題ないため、`name`属性のほうが便利な局面もあるかもしれません。

また、`form`属性を利用することで、この要素に関連するフォームを明示的に指定できます。以下は、`form`要素の外にある`output`要素を参照する例です。
...
この`output`要素は`form`要素の外にありますが、`form`属性でフォームと関連づけられているため、フォーム内のスクリプトから`this.result`として参照することができます。
...
<!-- a11y note -->
`output`要素のデフォルトのARIAロールは"status"です。statusロールは「ライブリージョン」(live region)であり、値が変化した際に支援技術に通知します。`output`要素の内容が変化すると、スクリーンリーダーはその内容を読み上げます。
