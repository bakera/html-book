...
`select`要素は、オプションから選択するためのコントロールを表します。子要素として、`option`要素で選択肢を記述します。以下はペットを選択させる例です。
...
`size`属性を指定すると、ユーザーに一度に見せる選択肢の数を指定できます。属性を指定しない場合のデフォルト値は"1"で、この場合、多くのブラウザではプルダウン式のリストとして表現されます。モバイル端末ではドラムロール型のUIで表現されることもあります。UIの表現方法は仕様で規定されていないため、他の表現方法になる可能性もあります。
...
`size`属性に"2"以上の値を指定すると、多くのブラウザでは高さが広がり、複数の選択肢が一度に見えるようになります。
...
フォーム送信を行うと、選択されている`option`要素の`value`属性の値がサーバーに送信されます。より詳しくは`option`要素の項を参照してください。
...
この属性を省略した場合のデフォルト値は、要素の内容となります。選択を行わない場合の初期値に対しては、`value=""`として明示的に空の値を指定する必要があります。
...
option要素の内容モデルはテキストです。マークアップを含めることはできません。



### `optgroup`要素

`optgroup`要素を使用することで、`option`要素による選択肢をグループ化することもできます。`optgroup`要素には`label`属性を指定する必要があり、これがグループの名前となります。
