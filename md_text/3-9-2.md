...
`type`属性のデフォルト値は`submit`です。`type`属性を指定しない場合、何もしないボタンではなく、送信ボタンになることに注意してください。フォーム送信を意図しないボタンを設置する場合は、明示的に`type=button`を指定する必要があります。

<!-- 内容モデル -->
`button`要素の内側モデルは"Phrasing"です。で要素（フレージングコンテンツ）を置くこともできることになります。

`input`要素でもボタンの機能を実現できますが、`input`要素のボタンの内容が`value`属性に指定した文字列になるのに対し、`button`要素では要素の内容がボタンのコンテンツとなります。言い換えると、
